...
export const Users: FC<UsersPropsType> = (
  {
    ...
    invites, onClickInvites, onClickInvite
  }) => {
  return (
    <>
      ...
      {isLoading ? (
        ...
        <ul className="users-list">
          {items
            ...
            .map(obj => <User onClickInvites={onClickInvites} isInvited={invites.includes(obj.id)}
                              key={obj.id} {...obj} />)}
        </ul>
      )}
      {invites.length > 0 && (
        <button className="send-invite-btn" onClick={onClickInvite}>Отправить приглашение</button>
      )}
    </>
  );
};
